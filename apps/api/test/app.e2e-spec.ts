...
describe('API (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    const moduleRef = await Test.createTestingModule({
      controllers: [HealthController, UsersController, ClaimsController],
      providers: [
        UsersService,
        ClaimsService,
  ...
  describe('GET /api/users', () => {
    it('returns a page of users with claim counts and pagination meta', async () => {
      ...
      const res = await http().get('/api/users').expect(200);
    ...
    it('coerces and forwards page/limit query params to the repository', async () => {
      ...
      await http().get('/api/users?page=3&limit=10').expect(200);
    ...
    it('rejects a non-positive page with 400', async () => {
      await http().get('/api/users?page=0').expect(400);
    });
  ...
  describe('GET /api/users/:id/claims', () => {
    it('404s for an unknown user', async () => {
      ...
      await http().get(`/api/users/${USER_ID}/claims`).expect(404);
    });
  });
  ...
  describe('GET /api/claims/:id', () => {
    ...
    it('404s when the claim is missing', async () => {
      ...
      await http().get(`/api/claims/${CLAIM_ID}`).expect(404);
    });

    it('returns the claim when found', async () => {
      claims.findOne!.mockResolvedValue(makeClaim());
      const res = await http().get(`/api/claims/${CLAIM_ID}`).expect(200);
  ...
  describe('GET /api/claims (filters)', () => {
    it('rejects an invalid status filter with 400', async () => {
      await http().get('/api/claims?status=PENDING').expect(400);
    });
  ...
  describe('POST /api/claims', () => {
    ...
    it('400s on a body that fails validation', async () => {
      await http().post('/api/claims').send({ type: 'COLLISION' }).expect(400);
    });
    ...
    it('400s when the referenced vehicle is missing', async () => {
      ...
      await http().post('/api/claims').send(validBody).expect(400);
    });
    ...
    it('creates a claim and returns 201', async () => {
      ...
      const res = await http().post('/api/claims').send(validBody).expect(201);
